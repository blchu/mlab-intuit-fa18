...
Commandeered devices typically emit a loud tone that 's associated with a feature that helps users locate lost or stolen devices . iPhones and iPads also display the message : `` Device hacked by Oleg Pliss . For unlock device , you need send voucher code by 100 usd/eur -LRB- Moneypack/Ukash/PaySafeCard -RRB- to email:lock404@hotmail.com for unlock . ''

In some cases -- specifically , when a user has n't assigned a strong passcode to a locked device -- it can only be unlocked by performing a factory reset , which completely wipes all previously stored data and apps .
...
One participant in the online discussion theorized the mass compromise may have been the result of hacking domain name system -LRB- DNS -RRB- servers used by Australian service providers to translate human readable addresses such as Apple.com into the IP addresses Internet routers rely on .
...
Readers are reminded they can be permanently locked out of their Apple ID accounts , and possibly their iPhone or iPad when running iOS 7 with Find My iPhone turned on , if they are compromised before two-factor authentication is enabled . Two-factor authentication wo n't automatically prevent an attacker from compromising an iCloud account , but it will prevent the attacker from changing security questions and other crucial settings in the event of a breach .
...
People with a locked device should immediately try changing the credentials for their Apple ID and ensure two-factor authentication is set up . In the event their locked device did n't have a passcode associated with it , they can perform a factory reset by using a cable to plug the device into their computer while iTunes is open .
